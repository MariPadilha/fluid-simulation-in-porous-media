...
__global__ void calc_upwind_Ui(double *dev_fn, double *dev_fs, double *dev_fe, double *dev_fw, double *dev_vm, double *dev_um, double *dev_areau_n, 
    ...
    double *dev_dxdvdy, double *dev_q_art, double *dev_xm, double *dev_liga_poros, double re, double **p, double *dev_ru, int imax, int jmax,
    ...
    if(i <= imax){
        ...
        dev_q_art[idx] = dev_epsilon1[idx] * (p[i][j]-p[i-1][j]) / (dev_x[i]-dev_x[i-1]) - b_art * (dev_dudxdx[idx]+dev_dxdvdy[idx]);
...
void upwind_Ui(double *dev_um, double *dev_vm, double **p, double *dev_ru, int j){
    ...
    calc_upwind_Ui<<<blocks, threads>>>(dev_fn, dev_fs, dev_fe, dev_fw, dev_vm, dev_um, dev_areau_n, dev_areau_s,
         ...
         dev_q_art, dev_xm, dev_liga_poros, re, p, dev_ru, imax, jmax, j, iterations.b_art, darcy_number, cf, g);
}