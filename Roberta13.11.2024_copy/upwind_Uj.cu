...
//--- upwind_U ---
__global__ void calc_upwind_Uj(double *dev_fn, double *dev_fs, double *dev_fe, double *dev_fw, double **vm, double **um, double *dev_areau_n, 
    ...
    double *dev_dxdvdy, double *dev_q_art, double *dev_xm, double *dev_liga_poros, double re, double **p, double **ru, int jmax, 
    ...
    if(j <= jmax-1){
        ...
        dev_q_art[idx] = dev_epsilon1[idx] * (p[i][j]-p[i-1][j]) / (dev_x[i]-dev_x[i-1]) 
...
void upwind_Uj(double **um, double **vm, double **p, double **ru, int i){
    ...
    calc_upwind_Uj<<<blocks, threads>>>(dev_fn, dev_fs, dev_fe, dev_fw, vm, um, dev_areau_n, dev_areau_s,
         ...
         dev_q_art, dev_xm, dev_liga_poros, re, p, ru, jmax, i, iterations.b_art, darcy_number, cf, g);
