...
__global__ void calc_resz(double *dev_dzudx, double *dev_dzvdy, double *dev_de, double *dev_dw, double *dev_dn, double *dev_ds, double *dev_dp, 
    ...
    if(i <= imax-1 && j <= jmax-1){
        dev_dzudx[idx] = 0.5 * (dev_z[(i+1)*(jmax+1)+j]+dev_z[idx]) * dev_um_n[(i+1)*(jmax+1)+j] * dev_areau_e[j]
                    - 0.5 * (dev_z[(i-1)*(jmax+1)+j]+dev_z[idx]) * dev_um_n[idx] * dev_areau_w[j];
        dev_dzvdy[idx] = 0.5 * (dev_z[i*(jmax+1)+(j+1)]+dev_z[idx]) * dev_vm_n[i*(jmax+2)+(j+1)] * dev_areav_n[i]
        ...
        dev_rz[idx] = 1.0 / (dev_xm[i+1]-dev_xm[i]) / (dev_ym[j+1]-dev_ym[j]) 
                *  (-dev_dp[idx]*dev_z[idx] + dev_de[idx]*dev_z[(i+1)*(jmax+1)+j] 
                +  dev_dw[idx]*dev_z[(i-1)*(jmax+1)+j] + dev_dn[idx]*dev_z[i*(jmax+1)+(j+1)] 
                +  dev_ds[idx]*dev_z[i*(jmax+1)+(j-1)] - (1.0-dev_liga_poros[idx])
