...
__global__ void calc_resc(double *dev_areau_e, double *dev_areau_w, double *dev_areav_n, double *dev_areav_s, double *dev_xm, 
    ...
    if(i <= imax-1 && j <= jmax-1){
        ...
        de = (dev_ym[j+1]-dev_ym[j]) * (1.0/re/sc) / (dev_x[i+1]-dev_x[i]);
        dw = (dev_ym[j+1]-dev_ym[j]) * (1.0/re/sc) / (dev_x[i]-dev_x[i-1]);
        dn = (dev_xm[i+1]-dev_xm[i]) * (1.0/re/sc) / (dev_y[j+1]-dev_y[j]);
        ds = (dev_xm[i+1]-dev_xm[i]) * (1.0/re/sc) / (dev_y[j]-dev_y[j-1]);
        dp = de + dw + dn + ds;
