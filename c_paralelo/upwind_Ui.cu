...
__global__ void calc_upwind_Ui(double *dev_vm, double *dev_um, double *dev_areau_n, 
    ...
    if(i <= imax){
        ...
        q_art = dev_epsilon1[idx] * (dev_p[idx]-dev_p[(i-1)*(jmax+1)+j]) / (dev_x[i]-dev_x[i-1]) - b_art * (dudxdx+dxdvdy);
...
void upwind_Ui(double *dev_um, double *dev_vm, double *dev_p, double *dev_ru, int j){
    ...
    calc_upwind_Ui<<<blocks, threads>>>(dev_vm, dev_um, dev_areau_n, dev_areau_s,
         dev_areau_e, dev_areau_w, dev_epsilon1, dev_ym, dev_x, dev_y,
         dev_xm, dev_liga_poros, re, dev_p, dev_ru, imax, jmax, j, iterations.b_art, darcy_number, cf, g);
}